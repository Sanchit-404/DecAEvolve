...
class HuggingFaceLLM(LLM):
    # Optional: set from external main.py --peft_checkpoint (or env LLMSR_PEFT_CHECKPOINT) before pipeline.
    peft_checkpoint: str | None = None
    ...
    def __init__(self, samples_per_prompt: int, model_name: str = None, 
        ...
        _h = os.environ.get("LLMSR_COMPLETIONS_HOST", "127.0.0.1")
        _p = os.environ.get("LLMSR_COMPLETIONS_PORT", "5000")
        url = f"http://{_h}:{_p}"
        self._url = url
        ...
        try:
            ...
            _peft = (getattr(HuggingFaceLLM, "peft_checkpoint", None) or "").strip()
            if not _peft:
                _peft = os.environ.get("LLMSR_PEFT_CHECKPOINT", "").strip()
            if _peft:
                if not os.path.isdir(_peft):
                    raise RuntimeError(f"PEFT checkpoint path is not a directory: {_peft}")
                from peft import PeftModel

                self.model = PeftModel.from_pretrained(self.model, _peft)
                print(f"Loaded PEFT adapter from {_peft}")
        ...
        # breakpoint()
        # Llama tokenizers often omit pad_token; tokenizer(..., padding=True) in _do_request then raises.
        if "llama" in model_name.lower() and self.tokenizer.pad_token is None:
    ...
    def _draw_samples_local(self, prompt: str, config: config_lib.Config) -> Collection[str]:
        ...
        prompt = '\n'.join([self._instruction_prompt, prompt])
        # trl vllm-serve (used by GRPO) does not expose OpenAI /v1/chat/completions — do not auto-route here.
        # Opt-in only: LLMSR_SAMPLE_VIA_OPENAI_VLLM=1 and LLMSR_COMPLETIONS_PORT or VLLM_SERVER_PORT (plain vLLM).
        use_openai_vllm = os.environ.get("LLMSR_SAMPLE_VIA_OPENAI_VLLM", "").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        if use_openai_vllm:
            vllm_port = (
                os.environ.get("LLMSR_COMPLETIONS_PORT", "").strip()
                or os.environ.get("VLLM_SERVER_PORT", "").strip()
            )
            if vllm_port:
                _h = os.environ.get("LLMSR_COMPLETIONS_HOST", "127.0.0.1")
                self._url = f"http://{_h}:{vllm_port}"

        last_err: BaseException | None = None
        for _attempt in range(5):
            try:
                all_samples = []
                if use_openai_vllm and vllm_port:
                    vres = self._do_request_vllm(prompt)
                    if self._batch_inference:
                        all_samples = list(vres) if isinstance(vres, list) else [vres]
                    else:
                        all_samples = [vres] if isinstance(vres, str) else list(vres)
                elif self._batch_inference:
                    response = self._do_request(prompt)
                    for res in response:
                        all_samples.append(res)
                else:
                ...
                continue
        raise RuntimeError(
            "draw_samples failed after 5 attempts (last error attached)."
        ) from last_err
    ...
    def _do_request_vllm(self, content: str) -> str:
        ...
        data = {
            # vLLM OpenAI server expects the served model id (HF name for trl vllm-serve --model).
    ...
    def _do_request(self, content: str) -> str:
        ...
        # Move inputs to the same device as the model (device_map may place weights on GPU or CPU).
        _dev = next(self.model.parameters()).device
        inputs = {k: v.to(_dev) for k, v in inputs.items()}
        
        with torch.no_grad():
            if self._batch_inference:
                # One giant num_return_sequences=N (e.g. 50) on 8B is slow/OOM; chunk (env LLMSR_MAX_PARALLEL_GENERATIONS, default 8).
                try:
                    gen_chunk = int(os.environ.get("LLMSR_MAX_PARALLEL_GENERATIONS", "8").strip())
                except ValueError:
                    gen_chunk = 8
                gen_chunk = max(1, gen_chunk)
                pad_id = self.tokenizer.pad_token_id
                if pad_id is None:
                    pad_id = self.tokenizer.eos_token_id
                eos_id = self.tokenizer.eos_token_id
                responses = []
                remaining = repeat_prompt
                while remaining > 0:
                    n_now = min(gen_chunk, remaining)
                    outputs = self.model.generate(
                        **inputs,
                        max_new_tokens=2048,
                        num_return_sequences=n_now,
                        do_sample=True,
                        temperature=0.8,
                        top_p=0.9,
                        top_k=50,
